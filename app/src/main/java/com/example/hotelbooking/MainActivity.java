package com.example.hotelbooking;

import androidx.appcompat.app.AppCompatActivity;

import android.app.DatePickerDialog;
import android.os.Bundle;
import android.text.TextUtils;
import android.view.View;
import android.widget.ArrayAdapter;
import android.widget.Button;
import android.widget.DatePicker;
import android.widget.EditText;
import android.widget.Spinner;
import android.widget.TextView;
import android.widget.Toast;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.Calendar;
import java.util.Date;

public class MainActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    TextView tvlocation,tvroom,tvcheckin,tvcheckout, tvlocation1,tvroomtype1, tvtotalroom, tvservicec,  tvvat, tvouutputcheckin, tvoutputcheckout,tvtoal;
    Spinner spinlocation,spinroom;
    Button btncalculate;
    EditText etchildren,etadult,etroom;
    Boolean a,b;

private void loadDatapicker(){
    final Calendar c = Calendar.getInstance();
    int year = c.get(c.YEAR);
    int month = c.get(c.MONTH);
    int day = c.get(c.DAY_OF_MONTH);

    DatePickerDialog datePickerDialog = new DatePickerDialog(
            this, new DatePickerDialog.OnDateSetListener() {
        @Override
        public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
            month = month + 1;
            String date = month + "/" + dayOfMonth + "/" + year;
            if (a == true){
                tvcheckin.setText(date);
                a = false;
            }
            else if (b == true)
            {
                tvcheckout.setText(date);
                b = true;
            }
        }
    },year,month,day);
    datePickerDialog.show();
}



    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        //binding
        tvlocation=findViewById(R.id.tvlocation);
        tvroom=findViewById(R.id.tvroom);
        tvcheckin=findViewById(R.id.tvcheckin);
        tvcheckout=findViewById(R.id.tvcheckout);

        spinlocation=findViewById(R.id.spinlocation);
        spinroom=findViewById(R.id.spinroom);
        btncalculate=findViewById(R.id.btncalculate);
        etchildren=findViewById(R.id.etchildren);
        etadult=findViewById(R.id.etadult);
        etroom=findViewById(R.id.etroom);
        tvlocation1 = findViewById(R.id.tvlocation1);
        tvroomtype1 = findViewById(R.id.tvroom1);
        tvtotalroom = findViewById(R.id.tvnoofroom1);
        tvservicec = findViewById(R.id.tvservice);
        tvvat = findViewById(R.id.tvvat);
        tvouutputcheckin = findViewById(R.id.tvcheckin1);
        tvoutputcheckout = findViewById(R.id.tvcCheckout1);
        tvtoal=findViewById(R.id.tvtotal);

        String location[] = {"kathmandu", "bhaktapur", "dharan",};
        ArrayAdapter adapter = new ArrayAdapter<>(
                this,
                android.R.layout.simple_list_item_1,
                location
        );

        spinlocation.setAdapter(adapter);

        String rooms[] = {"AC", "Platinium", "Deluxe", };
        ArrayAdapter adapter1 = new ArrayAdapter<>(
                this,
                android.R.layout.simple_list_item_1,
                rooms
        );

        spinroom.setAdapter(adapter1);

        tvcheckin.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                loadDatapicker();
                a = true;
            }
        });

        tvcheckout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                loadDatapicker();
                b = true;
            }
        });

        btncalculate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                if (TextUtils.isEmpty(tvcheckin.getText())){
                    tvcheckin.setError("Enter CheckIn Date");
                    return;
                }

                if (TextUtils.isEmpty(tvcheckout.getText())){
                    tvcheckout.setError("Enter CheckOut Date");
                    return;
                }

                if (TextUtils.isEmpty(etadult.getText())){
                    etadult.setError("Enter No of Adults");
                    return;
                }

                if (TextUtils.isEmpty(etchildren.getText())){
                    etchildren.setError("Enter No of Children");
                    return;
                }

                if (TextUtils.isEmpty(etroom.getText())){
                    etroom.setError("Enter No of Rooms");
                    return;
                }

                String RoomType = spinroom.getSelectedItem().toString();
                String Place =spinlocation.getSelectedItem().toString();

                if (Place == "Select Location"){
                    Toast.makeText(MainActivity.this, "Please select your Location", Toast.LENGTH_SHORT).show();
                }

                String CheckIn = tvcheckin.getText().toString();
                String CheckOut = tvcheckout.getText().toString();
                int TotalRooms = Integer.parseInt(etroom.getText().toString());
                int RoomValue = 0 ;
                int Price;
                int TotalPrice;
                int Vat;
                int ServiceCharge;

                SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");
                try {
                    Date date1 = sdf.parse(CheckIn);
                    Date date2 = sdf.parse(CheckOut);
                    long diff = date2.getTime() - date1.getTime();
                    long diffDays = diff / (24 * 60 * 60 * 1000);
                    int days = (int) diffDays;

                    if (RoomType == "Deluxe"){
                        RoomValue = 2000;
                    }
                    else if (RoomType == "AC"){
                        RoomValue = 3000;
                    }
                    else if (RoomType == "Platinium"){
                        RoomValue = 4000;
                    }
                    else
                    {
                        Toast.makeText(MainActivity.this, "Please Select Room Type", Toast.LENGTH_SHORT).show();
                    }

                    Price = RoomValue * TotalRooms * days;
                    Vat = (Price * 13)/100;
                    ServiceCharge = (Price * 10)/100;
                    TotalPrice = Price + Vat + ServiceCharge;

                    tvlocation1.setText("Location : ".concat(Place));
                    tvroomtype1.setText("Room Type : ".concat(RoomType));
                    tvouutputcheckin.setText("CheckIn : ".concat(CheckIn));
                    tvoutputcheckout.setText("CheckOut : ".concat(CheckOut));
                    tvtotalroom.setText("Total Rooms: ".concat(Integer.toString(TotalRooms)));
                    tvservicec.setText("Service Charge: ".concat(Integer.toString(ServiceCharge)));
                    tvvat.setText("Vat: ".concat(Integer.toString(Vat)));
                    tvtoal.setText("Total : " .concat(Integer.toString(TotalPrice)));
                } catch (ParseException e){
                    e.printStackTrace();
                }
            }
        });

    }



    @Override
    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
    String date="Checkindate : "+month+"/"+ dayOfMonth+"/" + year;
    tvcheckin.setText(date);
    tvcheckout.setText(date);

    }

}
